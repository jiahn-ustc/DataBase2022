<template>
  <el-form style="width: 30%" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="150px" class="demo-ruleForm">
    ...
    <el-form-item label="透支额" >
      <el-input v-model="ruleForm.overdraft"></el-input>
    </el-form-item>
...
export default {
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          axios.post('http://localhost:8181/checkAccount/update',this.ruleForm).then(function (resp){
            //console.log(resp);
            //alert(resp.data);
            if(resp.data=='success')
            {
              _this.$alert(_this.ruleForm.account_id+" 修改成功",'消息',{
                ...
                callback: action => {
                  _this.$router.push('/checkAccount');
  ...
  created() {
    ...
    const _this = this
    axios.get('http://localhost:8181/checkAccount/findById/'+this.$route.query.account_id).then(function (resp){

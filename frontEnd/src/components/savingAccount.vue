<template>
  <div>
    ...
    <el-form style="width: 30%" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="150px" class="demo-ruleForm">
      ...
      <el-form-item label="账户利率" prop="rate">
        <el-input v-model="ruleForm.rate"></el-input>
      </el-form-item>
      <el-form-item label="账户货币类型" prop="currency_type">
    ...
    <el-form style="width: 30%" :model="searchRuleForm" :rules="rules" ref="searchRuleForm" label-width="150px" class="demo-ruleForm">
      <el-form-item label="储蓄账户号" prop="account_id">
      ...
      <el-form-item label="利率">
        <el-input v-model="searchRuleForm.rate" readonly></el-input>
      </el-form-item>
      <el-form-item label="货币类型">
        <el-input v-model="searchRuleForm.currency_type" readonly></el-input>
      </el-form-item>
...
export default {
  methods: {
    search(){
      ...
      if(_this.searchRuleForm.account_id==='')
      {
        ...
        axios.get('http://localhost:8181/savingAccount/findById/'+_this.searchRuleForm.account_id).then(function (resp){

          _this.searchRuleForm = resp.data;
          console.log(_this.searchRuleForm);
          if(_this.searchRuleForm.bank_name==null)
          {
            _this.$alert("数据库中没有账户号为"+_this.searchRuleForm.account_id+"的储蓄账户，查询失败！！",'消息',{
              confirmButtonText: '确定'
    ...
    deleteAccount(row){
      ...
      const _this = this
      axios.delete('http://localhost:8181/savingAccount/deleteById/'+row.account_id).then(function (resp){
    ...
    page(currentPage){
      const _this = this
      axios.get('http://localhost:8181/savingAccount/findAll/'+currentPage+'/'+_this.size+'').then(function (resp){
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          axios.post('http://localhost:8181/savingAccount/save',this.ruleForm).then(function (resp){
            console.log(resp);
            //alert(resp.data);
  ...
  created() {
    const _this = this
    axios.get('http://localhost:8181/savingAccount/findAll/1/'+_this.size+'').then(function (resp){
      _this.tableData = resp.data.content
      _this.total = resp.data.totalElements
      //_this.size = resp.data.size
      console.log(resp)
